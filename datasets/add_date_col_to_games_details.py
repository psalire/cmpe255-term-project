...
player_stats_df.insert(0, 'DATE', '')

print('Start...')
for i in range(len(player_stats_df)):
    player_stats_df['DATE'].iat[i] = games_stats_df[
        games_stats_df['GAME_ID']==player_stats_df['GAME_ID'].iat[i]
    ]['DATE'].to_string(header=False, index=False)
    # if (i+1)%10==0:
    #     break
    if i%10000==0:
        print(i)
...
player_stats_df.sort_values(by=['DATE']).to_csv('out.csv', index=False)
